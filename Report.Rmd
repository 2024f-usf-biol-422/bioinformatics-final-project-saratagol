...
sra_data <- sra_data %>%
  mutate(
    treatment = case_when(
      ...
      grepl("vehicle", Sample.Name, ignore.case = TRUE) ~ "Vehicle",
...
vcf_with_metadata %>%
  ...
  ggplot(aes(x = lab_host, y = sample_count, fill = lab_host)) +
...
```{r treatment-vs-viral-load, fig.cap="Comparison of Treatment Type vs. Viral Load"}
# Plot: Comparison of Treatment Type vs. Viral Load
sra_data %>%
  filter(!is.na(treatment), !is.na(viral_load)) %>%
  ggplot(aes(x = treatment, y = viral_load, fill = treatment)) +
  geom_boxplot(outlier.shape = NA) + # Boxplot for distribution
  geom_jitter(width = 0.2, alpha = 0.5, color = "black") + # Add individual points
  labs(
    title = "Comparison of Treatment Type vs. Viral Load",
    x = "Treatment Type",
    y = "Viral Load (log scale)"
  ) +
  scale_y_log10() + # Use log scale for viral load
  theme_minimal() +
